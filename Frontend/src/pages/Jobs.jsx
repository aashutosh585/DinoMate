...
export default function Jobs() {
  ...
  const displayedJobs = jobs.filter((job) => {
    if (sourceFilter === "PORTAL") return job.jobSource !== "AGGREGATED";
    if (sourceFilter === "AGGREGATED") return job.jobSource === "AGGREGATED";
    return true;
  });
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 mt-2">
        <h2 className="text-2xl font-black text-slate-900 flex items-center gap-2">
          ...
          <span className="text-xs bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full font-bold">
            {displayedJobs.length} visible
          </span>
        </h2>

        {/* 2 Types of Jobs Filter Tabs */}
        <div className="inline-flex rounded-lg bg-slate-100 p-1 border border-slate-200 text-xs font-semibold">
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-md transition flex items-center gap-1 ${
              sourceFilter === "AGGREGATED"
                ? "bg-purple-700 text-white shadow-sm"
                : "text-slate-600 hover:text-purple-700"
            }`}
          >
            <span>🌐</span> 150+ Aggregated
          </button>
        </div>
      </div>
